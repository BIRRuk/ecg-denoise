...
ckpt_dir, plot_dir = [os.path.join(os.path.realpath(preset['cache_dir']), i) for i in cache_fldrs]
preset['ckpt_dir'] = ckpt_dir
from utils.utils import check_folder
for i in [os.path.realpath(preset['cache_dir']), ckpt_dir, plot_dir ]:
    check_folder(i)
...
pjson(preset, title=colorize('configs('+machine+')', tcolors.HEADER))
