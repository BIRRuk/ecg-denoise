...
def fit(
    ...
    if ckpt_path is not None:
        ...
        if args.modify_net:
            ...
            # net = modify_net_(net)
            # print(net)
            # for i in net.combine_modules:
            #     print(i.conv.weight)
            # optimizer = torch.optim.Adam(params=net.parameters(),  lr=args.lr)
            for param in optimizer.param_groups:
                param['lr'] = args.lr#*args.scheduler_factor
                # param['lr'] = param['lr']*args.scheduler_factor
                pass
    ...
    # for i in range(3):
    #     print(net.conv1[0].tl[i][0])
    #     net.conv1[0].tl[i][0].requires_grad_(False)
    # for i in range(3):
    #     print(net.seed.tl[i][0])
    #     net.seed.tl[i][0].requires_grad_(False)
    # for i in range(3):
    #     print(net.seed[0].tl[i][0])
    #     net.seed[0].tl[i][0].requires_grad_(False)
    # net.seed.tl[0][0].requires_grad_(False)
    # net.seed.tl[1][0].requires_grad_(False)
    # net.seed.tl[2][0].requires_grad_(False)
    # net.seed.tl[3][0].requires_grad_(False)
    net.seed.requires_grad_(False)
    # exit()
